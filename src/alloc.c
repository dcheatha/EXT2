 ...
 * @param directory
 */
void allocateDirectoryEntry(DiskInfo *disk_info, int32_t inode_start,
                            Directory *directory) {
  INode root_inode;
  Directory root_dir;
  ...
  while (1) {
    ...
    if (isEndDirectory(&root_dir)) {
      ...
      write_index =
          writeDirectory(disk_info, &root_inode, directory, write_index);
      // Now write the end dir back:
      write_index =
          writeDirectory(disk_info, &root_inode, &root_dir, write_index);
...
int32_t allocateINode(DiskInfo *disk_info, ExtInfo *ext_info) {
  ...
  for (int32_t group = 0; group < disk_info->group_count; group++) {
    ...
    for (int32_t pos = 0; pos < disk_info->inodes_per_group / 8; pos++) {
      ...
      if (bit != -1) {
        int32_t free_inode_pos =
            1 + bit + (pos * 8) + (group * disk_info->inodes_per_group);
